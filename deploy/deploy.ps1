...
.DESCRIPTION
    Deploys the AD Lab in phases using Azure CLI:
      ...
      6. Groups     - Create OU structure, 100 AD users, 18 department security groups (ADLab/Groups)
.PARAMETER Phase
...
$config = @{
    ...
    KvEast              = 'kv-mc-ad-east'
    KvCentral           = 'kv-mc-central'
...
$allVMs = @(
    ...
    @{ RG = $config.RgCentral; Name = 'DVDC03' }
...
"@ | Set-Content -Path $tempScript -Encoding UTF8
            ...
            # az vm user update is not supported on Domain Controllers — skip silently
            Write-Host "  (Skipping vm user update — not supported on DCs)" -ForegroundColor Gray

            # Update Key Vault secrets in both regions
            az keyvault secret set --vault-name $config.KvEast    --name 'vm-admin-password' --value $AdminPassword --output none 2>$null
            Write-Host "  Updated secret in $($config.KvEast)" -ForegroundColor Green
            az keyvault secret set --vault-name $config.KvCentral --name 'vm-admin-password' --value $AdminPassword --output none 2>$null
            Write-Host "  Updated secret in $($config.KvCentral)" -ForegroundColor Green
...
'@ | Set-Content -Path $tempValidate -Encoding UTF8
        ...
        5 {
            ...
            # Domain join via run-command (avoids CSE/SAS token shell-escaping issues)
            $memberVMs = @(
                @{ RG = $config.RgEast;    Name = 'DVAS01'; DcIp = $config.PrimaryDcIp }
                @{ RG = $config.RgEast;    Name = 'DVAS02'; DcIp = $config.PrimaryDcIp }
                @{ RG = $config.RgCentral; Name = 'DVAS03'; DcIp = $config.CentralDcIp }
                @{ RG = $config.RgCentral; Name = 'DVAS04'; DcIp = $config.CentralDcIp }
            )

            $joinScriptPath = Join-Path $config.ScriptsPath 'Join-DomainAndConfigure.ps1'
            $joinScriptContent = Get-Content $joinScriptPath -Raw

            Write-Host "`n── Domain joining member servers via run-command ──" -ForegroundColor Yellow

            foreach ($vm in $memberVMs) {
                Write-Host "  Joining $($vm.Name) to $($config.DomainName)..." -ForegroundColor Yellow

                $tempJoin = Join-Path $env:TEMP "join-$($vm.Name).ps1"
                "& {`n$joinScriptContent`n} -DomainName '$($config.DomainName)' -DcIpAddress '$($vm.DcIp)' -DomainAdminUser '$($config.AdminUsername)' -DomainAdminPassword '$AdminPassword'" |
                    Set-Content -Path $tempJoin -Encoding UTF8

                $joinResult = az vm run-command invoke `
                    --resource-group $vm.RG `
                    --name $vm.Name `
                    --command-id RunPowerShellScript `
                    --scripts "@$tempJoin" `
                ...
                Remove-Item $tempJoin -Force -ErrorAction SilentlyContinue

                if ($LASTEXITCODE -ne 0) {
                    Write-Host "  $($vm.Name) — run-command exited (VM may be rebooting after join)" -ForegroundColor Yellow
                } else {
                    $parsed = $joinResult | ConvertFrom-Json
                    $stdout = ($parsed.value | Where-Object { $_.code -eq 'ComponentStatus/StdOut/succeeded' }).message
                    if ($stdout) { Write-Host "  $($stdout.Trim())" -ForegroundColor Gray }
                }
            }
        ...
        6 {
            ...
            # ── Step 6a: Create OU structure ──
            Write-Host "`n── Step 6a: Creating OU structure on DVDC01 ──" -ForegroundColor Yellow
            $ouScriptContent = Get-Content (Join-Path $config.ScriptsPath 'Configure-OUs.ps1') -Raw
            $tempOU = Join-Path $env:TEMP 'run-ous-remote.ps1'
            "& {$nl$ouScriptContent$nl} -DomainDN '$($config.DomainDN)'" |
                Set-Content -Path $tempOU -Encoding UTF8
            $ouResult = az vm run-command invoke --resource-group $config.RgEast --name 'DVDC01' --command-id RunPowerShellScript --scripts "@$tempOU" -o json 2>&1
            Remove-Item $tempOU -Force -ErrorAction SilentlyContinue
            if ($LASTEXITCODE -eq 0) {
                $parsed = $ouResult | ConvertFrom-Json
                $stdout = ($parsed.value | Where-Object { $_.code -eq 'ComponentStatus/StdOut/succeeded' }).message
                if ($stdout) { Write-Host $stdout -ForegroundColor Cyan }
                Write-Host "  [OK] OU structure ready." -ForegroundColor Green
            } else {
                Write-Warning "Configure-OUs may have failed — continuing anyway (OUs may already exist)."
            }

            # ── Step 6b: Upload CSV and create AD users ──
            Write-Host "`n── Step 6b: Uploading users.csv and creating AD users ──" -ForegroundColor Yellow
            $storageKey = az storage account keys list --resource-group $config.RgEast --account-name $config.StorageAccountName --query '[0].value' -o tsv
            az storage blob upload --account-name $config.StorageAccountName --account-key $storageKey --container-name 'scripts' --name 'users.csv' --file $config.CsvPath --overwrite --output none
            $csvExpiry = (Get-Date).ToUniversalTime().AddHours(4).ToString('yyyy-MM-ddTHH:mmZ')
            $csvSasToken = az storage blob generate-sas --account-name $config.StorageAccountName --account-key $storageKey --container-name 'scripts' --name 'users.csv' --permissions r --expiry $csvExpiry -o tsv
            $csvSasUrl = "https://$($config.StorageAccountName).blob.core.windows.net/scripts/users.csv?$csvSasToken"

            $dlScript = @'
New-Item -Path C:\Temp -ItemType Directory -Force | Out-Null
$url = 'PLACEHOLDER_CSV_URL'
Invoke-WebRequest -Uri $url -OutFile 'C:\Temp\users.csv' -UseBasicParsing -ErrorAction Stop
Write-Output "CSV downloaded: $((Get-Item 'C:\Temp\users.csv').Length) bytes"
'@ -replace 'PLACEHOLDER_CSV_URL', $csvSasUrl
            $tempDl = Join-Path $env:TEMP 'dl-csv-remote.ps1'
            Set-Content -Path $tempDl -Value $dlScript -Encoding UTF8
            az vm run-command invoke --resource-group $config.RgEast --name 'DVDC01' --command-id RunPowerShellScript --scripts "@$tempDl" --output none 2>&1
            Remove-Item $tempDl -Force -ErrorAction SilentlyContinue

            $usersScriptContent = Get-Content (Join-Path $config.ScriptsPath 'Create-ADUsers.ps1') -Raw
            $tempUsers = Join-Path $env:TEMP 'run-users-remote.ps1'
            "& {$nl$usersScriptContent$nl} -CsvPath 'C:\Temp\users.csv' -DomainDN '$($config.DomainDN)'" |
                Set-Content -Path $tempUsers -Encoding UTF8
            $usersResult = az vm run-command invoke --resource-group $config.RgEast --name 'DVDC01' --command-id RunPowerShellScript --scripts "@$tempUsers" -o json 2>&1
            Remove-Item $tempUsers -Force -ErrorAction SilentlyContinue
            if ($LASTEXITCODE -eq 0) {
                $parsed = $usersResult | ConvertFrom-Json
                $stdout = ($parsed.value | Where-Object { $_.code -eq 'ComponentStatus/StdOut/succeeded' }).message
                if ($stdout) { Write-Host $stdout -ForegroundColor Cyan }
                Write-Host "  [OK] AD users created/verified." -ForegroundColor Green
            } else {
                Write-Warning "Create-ADUsers may have failed — check output above."
            }

            # ── Step 6c: Create department security groups ──
            Write-Host "`n── Step 6c: Running Create-DepartmentGroups.ps1 on DVDC01 ──" -ForegroundColor Yellow
            ...
            "& {$nl$scriptContent$nl} -DomainDN '$($config.DomainDN)'" |
...
Verification steps:
  1. az vm list -g rg-east    -o table    (4 VMs: DVDC01, DVDC02, DVAS01, DVAS02)
  2. az vm list -g rg-central -o table    (3 VMs: DVDC03, DVAS03, DVAS04)
  3. az network bastion list -o table      (1 Bastion in rg-east, Standard SKU)
  4. No public IPs on VMs
  5. East Bastion tunnel -> RDP to all VMs (peering enables Central access)
